...
class PendulumSimulation(eqx.Module):
    ...
    def render_pendulum(
        ...
        image = jnp.array(distance < self.ball_size).astype(jnp.int32)
    ...
    def generate_dataset(
        ...
        for i in range(n_sims):
            ...
            # Fill in detail here
            initial_angle = jax.random.uniform(key=key, minval=-jnp.pi, maxval=jnp.pi)
            # Fill in detail here
            initial_velocity = jax.random.uniform(key=key, minval=-1, maxval=1)
